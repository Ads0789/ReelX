import { Link, useLocation } from "react-router-dom";

function Sidebar() {
  const location = useLocation();

  const menuItems = [
    { path: "/", label: "Home", icon: "home" },
    { path: "/trending", label: "Trending", icon: "trending" },
    { path: "/subscriptions", label: "Subscriptions", icon: "subscriptions" },
  ];

  const libraryItems = [
    { path: "#", label: "Library", icon: "library" },
    { path: "#", label: "History", icon: "history" },
    { path: "#", label: "Your videos", icon: "video" },
    { path: "#", label: "Watch later", icon: "clock" },
    { path: "#", label: "Liked videos", icon: "like" },
  ];

  const subscriptions = [
    { name: "Tech Channel", avatar: "T" },
    { name: "Music Hub", avatar: "M" },
    { name: "Gaming Zone", avatar: "G" },
    { name: "News Today", avatar: "N" },
  ];

  const renderIcon = (iconType: string) => {
    switch (iconType) {
      case "home":
        return (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
          </svg>
        );
      case "trending":
        return (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
          </svg>
        );
      case "subscriptions":
        return (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
          </svg>
        );
      default:
        return (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
        );
    }
  };

  return (
    <aside className="fixed left-0 top-16 w-64 h-full bg-white border-r border-gray-200 overflow-y-auto">
      <div className="p-3">
        {/* Main Navigation */}
        <nav className="space-y-1 mb-6">
          {menuItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center space-x-6 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                location.pathname === item.path
                  ? "bg-gray-100 text-gray-900"
                  : "text-gray-700 hover:bg-gray-50"
              }`}
            >
              {renderIcon(item.icon)}
              <span>{item.label}</span>
            </Link>
          ))}
        </nav>

        <hr className="border-gray-200 mb-6" />

        {/* Library */}
        <div className="mb-6">
          <h3 className="px-3 mb-2 text-sm font-medium text-gray-500 uppercase tracking-wider">
            Library
          </h3>
          <nav className="space-y-1">
            {libraryItems.map((item) => (
              <Link
                key={item.label}
                to={item.path}
                className="flex items-center space-x-6 px-3 py-2 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
              >
                {renderIcon(item.icon)}
                <span>{item.label}</span>
              </Link>
            ))}
          </nav>
        </div>

        <hr className="border-gray-200 mb-6" />

        {/* Subscriptions */}
        <div>
          <h3 className="px-3 mb-2 text-sm font-medium text-gray-500 uppercase tracking-wider">
            Subscriptions
          </h3>
          <nav className="space-y-1">
            {subscriptions.map((channel) => (
              <Link
                key={channel.name}
                to={`/channel/${channel.name.toLowerCase().replace(" ", "-")}`}
                className="flex items-center space-x-3 px-3 py-2 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
              >
                <div className="w-6 h-6 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white text-xs font-bold">
                  {channel.avatar}
                </div>
                <span>{channel.name}</span>
              </Link>
            ))}
          </nav>
        </div>
      </div>
    </aside>
  );
}

export default Sidebar;